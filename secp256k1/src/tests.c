...
void test_ec_grind(int size, int use_generator) {
    ...
    secp256k1_pubkey pubs[17];
    unsigned char privs[17][32];
    ...
    for (i = 0; i < size * 2; ++i) {
        int pos = secp256k1_rand_int(size);
    ...
    secp256k1_ecmult(&ctx->ecmult_ctx, &result, NULL, NULL, &privs_acc);
